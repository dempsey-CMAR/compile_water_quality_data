...
Welcome to the [Centre for Marine Applied Research](https://cmar.ca/)'s (CMAR) Calibration-Validation App. The app provides a user interface to validate the calibration of sensors deployed as part of CMAR's [Coastal Monitoring Program](https://cmar.ca/coastal-monitoring-program/).
...
A deployment log with metadata about the validation must be saved in a folder called "log". The log must include a row for each sensor validated and the following columns:
